...
with tab3:
    ...
    if preview is not None:
        st.dataframe(preview[["Number", "Wine Name", "Vintage", "Price", "Sold"]],
...
with tab4:
    ...
    with a2:
        new_location  = st.text_input("Location", placeholder="e.g. Rack A1")
        new_astrisk   = st.selectbox("Notable (Astrisk)", [0, 1], format_func=lambda x: "Yes ★" if x == 1 else "No")
        new_sold      = st.number_input("Bottles Sold", min_value=0, step=1, value=0)
        new_bottles   = st.number_input("Number of Bottles", min_value=0, step=1, value=0)
...
with tab5:
    ...
    if edit_row is not None:
        ...
        with e2:
            e_location = st.text_input("Location",  value=str(r["Location"]) if pd.notna(r["Location"]) else "")
            e_astrisk  = st.selectbox("Notable (Astrisk)", [0, 1], index=int(r["Astrisk"]), format_func=lambda x: "Yes ★" if x == 1 else "No")
            e_sold     = st.number_input("Bottles Sold", min_value=0, step=1, value=int(r["Sold"]))
            e_bottles  = st.number_input("Number of Bottles", min_value=0, step=1, value=int(r["Number of Bottles"]))
